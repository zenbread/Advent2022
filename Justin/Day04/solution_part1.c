...
bool DoSectionsOverlap(uint64_t **section1, uint64_t **section2){
    ...
    uint64_t *overlapBuffer = NULL;

    overlapBuffer = calloc(sizeof(uint64_t), 2);
    if(NULL == overlapBuffer){
        printf("[-] Could not allocate overlap buffer\n");
        return false;
    }

    overlapBuffer[0] = ((*section1)[0] & (*section2)[0]);
    overlapBuffer[1] = ((*section1)[1] & (*section2)[1]);

    isOverlapped = (memcmp(overlapBuffer, (*section1), sizeof(uint64_t)*2) == 0);
    isOverlapped |= (memcmp(overlapBuffer, (*section2), sizeof(uint64_t)*2) == 0);

    if(isOverlapped){
        printf("[!] This is overlapped!\n");
    }
    printf("\n");
